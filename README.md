...
Evaluating public policy interventions is notoriously difficult because selection bias completely distorts the picture people who volunteer for job training already bring different motivations, education levels, and work histories to the table. Traditional regression methods blur these traits together, often yielding massively inflated estimates because they fail to properly account for the fact that participants usually start at a severe financial disadvantage.

To solve this, I applied a progressive suite of Causal Inference models (including Propensity Score Matching, Double Machine Learning, and Causal Forests) to the landmark Lalonde dataset. By residualizing out background noise and selection bias, my models stripped away the inflated baseline estimates to reveal the true, conservative Average Treatment Effect (ATE). Furthermore, my Causal Forest isolated Heterogeneous Treatment Effects (HTE) across different demographic profiles. Instead of simply asking whether a policy works on average, I used this approach to map how individual traits like past earnings drive personalized financial returns, transforming a blunt policy review into an actionable, targeted decision tool.
